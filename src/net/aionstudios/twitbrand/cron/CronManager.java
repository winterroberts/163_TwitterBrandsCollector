	...
	public static void startCron() {
		if(!cronStarted) {
			...
			cronThread = new Thread() {
			    public void run() {
			    	int min = DateTimeUtils.getCronMinute()-1;
			    	int hour, dom, month, dow, year = 0;
			    	while(cronStarted) {
			    		if(min!=DateTimeUtils.getCronMinute()) {
			    			min = DateTimeUtils.getCronMinute();
			    			hour = DateTimeUtils.getCronHour();
			    			dom = DateTimeUtils.getCronDayOfMonth();
			    			month = DateTimeUtils.getCronMonth();
			    			dow = DateTimeUtils.getCronDayOfWeek();
			    			year = DateTimeUtils.getCronYear();
